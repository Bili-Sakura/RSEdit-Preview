...
def log_validation(
    ...
    wandb_table = None
    if is_wandb_available():
        # Only initialize table if we have a wandb tracker
                ...
                if accelerator.is_main_process:
                    ...
                    for tracker in accelerator.trackers:
                        if tracker.name == "tensorboard":
                            import numpy as np
                            # Log original image (only once per image)
                            if sample_idx == 0:
...
def main():
    ...
    # column_names = dataset["train"].column_names  # Custom dataset might not have column_names attribute if PyTorch dataset
    ...
    def custom_collate_fn(batch):
        # Batch is a list of dicts from RSCCDataset
        # We need to convert it to the format expected by preprocess_train's logic (if we reused it)
        # or just reimplement logic here. Reimplementing is safer for PyTorch DataLoader.
        ...
        # Apply transforms (resize, crop, flip)
        # To ensure same random crop/flip, we concat
        
        # Convert to numpy and resize (as in convert_to_np)
        ...
        # Concat along channel dim? No, train.py stacks them in batch dim for transforms
        # original_images_np is list of (3, H, W)
        
        # Stack into (B, 3, H, W) -> No, train.py does np.concatenate to get (B*2, H, W, 3) or similar? 
        # convert_to_np returns (3, H, W)
        
        # Let's follow preprocess_images logic:
        # original_images = np.concatenate([convert...]) -> (B*3, H, W)? No. 
        # convert_to_np: (3, H, W). 
        # np.concatenate list of (3, H, W) -> (B*3, H, W) if axis 0.
        
        # Wait, preprocess_images in train.py:
        # original_images = np.concatenate([convert_to_np(image, args.resolution) for image in examples[original_image_column]])
        # If examples is dict of lists, this concat makes one big array.
        ...
        # Apply transforms
        # train_transforms expects (C, H, W) or batch? 
        # transforms.Compose with CenterCrop/RandomCrop works on (C, H, W) or (B, C, H, W)?
        # Usually PIL images. But here we have Tensor. 
        # train.py convert_to_np returns numpy array.
        # train.py preprocess_images converts to tensor then applies train_transforms.
        # Check train_transforms: CenterCrop, RandomHorizontalFlip. These work on Tensors too.
    ...
    if args.dataset_name is None:
         # Use custom RSCC dataset
         pass # train_dataset is already RSCCDataset instance
    else:
        # HF Dataset path
    ...
    def collate_fn(examples):
        # This is for HF Dataset which returns list of dicts from preprocess_train (if map style) 
        # or if we use torch DataLoader with map-style HF dataset, examples is list of results from preprocess_train.
        # preprocess_train returns dict with keys "original_pixel_values", etc.
        
        # If using RSCCDataset (custom), examples is list of dicts {input_image, ...}
        # But we want to use custom_collate_fn for RSCCDataset.
...
if __name__ == "__main__":
    ...
    # CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes=1 --mixed_precision=bf16 train_unet.py \
